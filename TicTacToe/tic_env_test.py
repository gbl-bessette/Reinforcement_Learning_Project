...
# Similar to TicTacToeEnv
class TictactoeEnvDQL:
    '''
    Description:
...
class Agent:
    ...
    # Transforms a grid array into an integer between 0 and 19683 (max number of possible grid states)
    def state2int(self, grid):
        state = grid.copy()
        state = state.reshape(3 * 3)
        state = np.where(state < 0, 2, state)
        return int(state[0]+3*state[1]+9*state[2]+27*state[3]+81*state[4]+243*state[5]+729*state[6]+2187*state[7]+6561*state[8])
    
    # Transforms coordinates as tuple into an integer between 0 and 8
    ...
    # Given a grid s and associated action a and the Q_table as input
    # Outputs the Q-value Q(s,a) in the table
    def Q_grid(self, grid, a, Q):
        nb = self.state2int(grid)
        index_a = self.action_index(a)
        return Q[nb,index_a]
    
    # Given a grid and the Q-table
    # Returns the best action as an int from available actions, following the epsilon greedy policy
    def act_Q_Learning(self, grid, Q, **kwargs):
        ...
        if random.random() < self.epsilon:
            ...
            q=[]
            actions = []
            for a in self.empty(grid):
                actions.append(a)
                q.append(self.Q_grid(grid,a,Q))
            index = np.argmax(q)
            a = actions[index]
    ...
    # Given a grid and the Q-table
    # Returns the maximum Q value of all available actions
    # Returns 0 if it is the end of the game
    def Max_Q_prime(self, grid_prime, Q, ends):
        if ends == False:
            q_prime=[]
            actions_prime = []
            nb_prime = self.state2int(grid_prime)
            for a in self.empty(grid_prime):
                actions_prime.append(a)
                index_a = self.action_index(a)
                q_prime.append(Q[nb_prime,a])
 
            return np.max(q_prime)
        else:
            return 0
    
    # Updates the Q values given a grid, maxQ, Q(s,a), alpha, gamma and reward rt
    def update_Q(self, grid, max_q_prime, q_s_a, alpha, gamma, rt):
        q_s_a = q_s_a + alpha*(rt + gamma*max_q_prime - q_s_a)
        return q_s_a
    
    # Counts the number of updated Q-values
    def nb_updated_Q(self, Q, Q_prime):
        diff  = Q_prime - Q
        return np.count_nonzero(diff)

    # Returns the mask to select the Q-values of avalable actions given a grid state_s
    def Mask(self, state_s):
        mask = state_s.copy()
        mask[np.nonzero(mask)] = True  
        return mask 

    # Returns the Q-values of avalable actions given a grid state_s and the Q-table Q
    def Q_heat(self,state_s,Q):
        s = state_s.copy()
        for a in self.empty(state_s):
            s[a] = Q[self.state2int(state_s)][self.action_index(a)]
        return s